...
public class JRangeSlider extends JComponent implements MouseListener,
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3163189464180615530L;

	private static final LinearGradientPaint gradientPaint = new LinearGradientPaint(
			0, 0, 280, 44, new float[] { .1F, .5F, .9F }, new Color[] {
	...
	public int tickIncrement = 1;
	public int labelIncrement = 1;
	...
	public void setLowValue(int lowValue) {
		...
		model.setRangeProperties(lowValue, e, model.getMinimum(),
	...
	public void paintComponent(Graphics g) {
		...
		int height = (int) bounds.getHeight() - 1 - (drawTicks ? 7 : 0)
				- (drawLabels ? 14 : 0);

		if (drawTicks) {
			// System.out.println("Printining Ticksk:=" + height + "/" +
			// bounds.height);
			paintTicks((Graphics2D) g);
		}
		if (drawLabels) {
		...
		if (empty) {
			if (direction == LEFTRIGHT_TOPBOTTOM) {
				min = ARROW_SZ;
				max = (orientation == VERTICAL) ? height - ARROW_SZ : width
						- ARROW_SZ;
			} else {
				min = (orientation == VERTICAL) ? height - ARROW_SZ : width
						- ARROW_SZ;
				max = ARROW_SZ;
			}
		}
	...
	protected void paintTicks(Graphics2D g) {
		...
		for (int i = model.getMinimum(); i <= model.getMaximum(); i += tickIncrement) {
			int x = toScreen(i);
	...
	protected void paintLabels(Graphics2D g) {
		...
		g.setColor(Color.white);
		for (int i = model.getMinimum(); i <= model.getMaximum(); i += labelIncrement) {
			int x = toScreen(i) - fm.stringWidth(Integer.toString(i)) / 2;
	...
	protected void paintArrow(Graphics2D g2, double x, double y, int w, int h,
		...
		if (orientation == VERTICAL) {
			...
			if (topDown) {
				...
				for (int i = 0; i < (w / 2 + 1); i++) {
					g2.drawLine(intX + w / 2 - i, intY + i, intX + w - w / 2
							+ i - 1, intY + i);
				}
			}
		} else {
			...
			if (topDown) {
				...
				for (int i = 0; i < (h / 2 + 1); i++) {
					g2.drawLine(intX + i, intY + h / 2 - i, intX + i, intY + h
							- h / 2 + i - 1);
				}
			}
		}
	...
	private void grow(int increment) {
		model.setRangeProperties(model.getValue() - increment,
				model.getExtent() + 2 * increment, model.getMinimum(),
				model.getMaximum(), false);
	}
